...
ObjectLoader.prototype.parse = function(json, onLoad)
{
	...
	var assetObjects = this.parseAssetObjects(json.assetObjects)
	var object = this.parseObject(json.object, geometries, materials, textures, audio, fonts, assetObjects)
...
ObjectLoader.prototype.parseAssetObjects = function(json) {
	...
	if (json !== undefined) {
		for(var i = 0; i < json.length; i++) {
			...
			assetObjects[object.uuid] = object
		}
	}
...
//Parse objects
ObjectLoader.prototype.parseObject = function(data, geometries, materials, textures, audio, fonts, assetObjects)
{
	...
	switch(data.type)
	{
		...
		case "OrthographicCamera":
			...
			if (data.clearDepth !== undefined) {
				object.clearDepth = data.clearDolor
			}
            if(data.order !== undefined) {
                object.order = data.order
            }
		...
		case "ObjectCaller":
			object = new ObjectCaller(data.objUuid, data.objType)
			break
	...
	if (data.blocksType !== undefined) {
		object.blocksType = data.blocksType
	}
	...
	if(data.type === "Program")
	{
		...
		object.assetObjects = assetObjects
